...
namespace cc
{
    ...
    public:
        ...
        math::Vector3f get_extent() const;
        std::array<math::Vector3f, 8> get_vertices() const;
        bool intersect(const AABB &box) const;

        bool inside(math::Vector3f const &p) const;
        bool inside(AABB const &bbx) const;
        ...
        auto transform(math::Matrix4f const &mat) -> void;
        auto center_and_extent() const -> std::tuple<math::Vector3f, math::Vector3f>;
        float x[2], y[2], z[2];
    };
}
